...
*Setup instructions for a gitian build of Tanzanite Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Tanzanite
...
to the Tanzanite GitHub release page.
...
- [Building Tanzanite Core](#building-tanzanite-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/tanzanite-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/tanzanite-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/tanzanite-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/tanzanite-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/tanzanite-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/tanzanite-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/tanzanite-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@tanzanite.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/tanzanite-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/tanzanite-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/tanzanite-osx-*-build.assert
```
