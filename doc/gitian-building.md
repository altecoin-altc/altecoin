...
*Setup instructions for a gitian build of Altecoin Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Altecoin
...
to the Altecoin GitHub release page.
...
- [Building Altecoin Core](#building-altecoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/altecoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/altecoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/altecoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/altecoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/altecoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/altecoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/altecoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@altecoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/altecoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/altecoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/altecoin-osx-*-build.assert
```
