...
- [Development guidelines](#development-guidelines)
    - [General Tanzanite Core](#general-tanzanite-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Tanzanite Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play TZTs (tTZT)" on the test network, if you
...
Tanzanite Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/tanzanite-project/tanzanite/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_tanzanite
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_tanzanite --log_level=test_suite
$ valgrind -v --leak-check=full src/tanzanited -printtoconsole
```
...
# A coverage report will now be accessible at `./test_tanzanite.coverage/index.html`.
```
...
- BitcoinMiner : Generates TZTs (if wallet is enabled).
...
However, in open source software such as Tanzanite Core, where everyone uses
...
pay attention to for reviewers of Tanzanite Core code.

General Tanzanite Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Tanzanite Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Tanzanite Core so that they can be integrated
...
file descriptors it will cause problems with Tanzanite's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:Tanzanite-Project/Tanzanite.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `tanzanite-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `tanzanite-cli` and the GUI debug console use this table to determine how to
