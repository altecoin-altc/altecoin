...
- [Development guidelines](#development-guidelines)
    - [General Altecoin Core](#general-altecoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Altecoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Altecoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/altecoin-project/altecoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_altecoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_altecoin --log_level=test_suite
$ valgrind -v --leak-check=full src/altecoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_altecoin.coverage/index.html`.
```
...
- BitcoinMiner : Generates ALTCs (if wallet is enabled).
...
However, in open source software such as Altecoin Core, where everyone uses
...
pay attention to for reviewers of Altecoin Core code.

General Altecoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Altecoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Altecoin Core so that they can be integrated
...
file descriptors it will cause problems with Altecoin's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:Altecoin-Project/Altecoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `altecoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `altecoin-cli` and the GUI debug console use this table to determine how to
