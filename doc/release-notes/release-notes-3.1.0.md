Tanzanite Core version *3.1.0* is now available from:  <https://github.com/tanzanite-project/tanzanite/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/tanzanite-project/tanzanite/issues>


Mandatory Update
==============

Tanzanite Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
Tanzanite Core is extensively tested on multiple operating systems using
...
Tanzanite Core should also work on most other Unix-like systems but is not
...
zTZT Staking is here! zTZT staking will be activated on the morning of the 8th of May 2018. With the release of zTZT staking, there are effectively 2 versions of zTZT, zTZT minted on the 3.0.6 Tanzanite wallet or lower, and zTZT minted on Tanzanite wallet version or higher. New features in this release will require the use of zTZT v2, zTZT minted on this wallet release 3.1.0 or later. If you currently hold zTZT v1 and wish to take advantage of zTZT staking and deterministic zTZT, you will need to spend the zTZT v1 to yourself and remint zTZT v2.
Note: To find your zTZT version, click the privacy tab, then the zTZT Control button then expand the arrows next to the desired denomination.


### Deterministic zTZT Seed Keys

zTZT is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zTZT outside of the wallet that the zTZT had been minted on. zTZT can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zTZT minting

zTZT minting now only requires 1 further mint (down from 2) to mature. zTZT mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zTZT Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zTZT denomination has been spent or not.



TZT/zTZT Staking and Masternode Rewards
--------------

### TZT, zTZT and Masternode Payment Schedule

To encourage the use of zTZT and increase the Tanzanite zerocoin anonymity set, the Tanzanite payment schedule has been changed to the following:

If a user staking zTZT wins the reward for their block, the following zTZT reward will be: 
- 3 zTZT (3 x 1 denominations) rewarded to the staker, 2 TZT rewarded to the masternode owner and 1 TZT available for the budget. This is a total block reward of 6 TZT, up from 5.

If a user staking TZT wins the reward, the following amounts will be rewarded: 
- 2 TZT to the TZT staker, 3 TZT to the Masternode owner and 1 TZT available for the budget. This is a total block reward of 6 TZT, up from 5.


### Return change to sender when minting zTZT

Previously, zTZT minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most TZT and return the change from a zTZT mint to that address.
...
The visual layout of the Tanzanite Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zTZT minting feature. This is enabled by default and the enablezeromint=0 setting in the tanzanite.conf file will overwrite the GUI option.
-	The percentage of autominted zTZT can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular TZT.
...
### Summary
 ...
 The Tanzanite wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the tanzanite.conf file, found in the Tanzanite data directory.
...
* If zTZT backup, auto generated name is `wallet-autozpivbackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
Tanzanite Daemon & Client (RPC Changes)
--------------
...
This allows running tanzanited without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the Tanzanite support group, which often times sees users say "I didn't spend that zTZT". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the Tanzanite masternodes, the monthly budget available to be utilised has been increased to 42,000 TZT / month. This TZT only has the opportunity to be raised once per month (paid to winning proposals) with any unused TZT not created by the blockchain.

### Budget Finalization Fee

The Tanzanite finalization fee for successful proposals has now been reduced, this fee is now 5 TZT down from 50 TZT. The total fee outlay for a successful proposal is now a total of 55 TZT.
...
-	Enforced v1 zTZT spends to require a security level of 100
-	Updates to zTZT spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the Tanzanite-Qt masternodes tab
-	Updated Tanzanite-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and Tanzanite's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zTZT Control dialog in the QT wallet, a user is able to see zTZT marked as version 1 or 2.

zPoS (zTZT staking)
--------------

Once a zTZT has over 200 confirmations it becomes available to stake. Staking zTZT will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zTZT. So for example if a 1,000 zTZT denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zTZTs.

Secure Spending
--------------

Version 1 zerocoins, as implemented by [Miers et. al](http://zerocoin.org/media/pdf/ZerocoinOakland.pdf), allow for something we describe as *serial trolling*. Spending zerocoins requires that the spender reveal their serial number associated with the zerocoin, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within Tanzanite's zerocoin implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new zerocoin with the same serial number, and spend the new zerocoin before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.
...
Zerocoins, or zTZT, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zTZT that can be recalculated at any time using the seed. Deterministic zTZT allows for users to backup all of their future zTZT by simply recording their seed and keeping it in a safe place (similar to backing up a private key for TZT). The zTZT seed needs to remain in the wallet in order to spend the zTZT after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zTZT data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

Tanzanite's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which Tanzanite used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to Tanzanite using the bug bounty program. The modulus is now correctly imported and Tanzanite's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] zTZT v2: zPoS dzTZT ezTZT external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zTZT balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zTZT status output (Mrs-X)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault tanzanite-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zTZT spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zTZT. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zTZT. (presstab)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/tanzanite-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.
