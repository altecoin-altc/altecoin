Altecoin Core version *v4.0.0* is now available from:  <https://github.com/altecoin-project/altecoin/releases>
...
Altecoin Core v4.0.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
You can read more details about this extensive work in ([PR #954](https://github.com/Altecoin-Project/Altecoin/pull/954))
...
A brand new feature is being introduced with the release of v4.0.0: Cold Staking ([PR #955](https://github.com/Altecoin-Project/Altecoin/pull/955))! This feature allows a coin owner to keep coins in a "cold" (or locked) wallet whilst a "hot" wallet carries out the burden of staking those coins.
...
A full technical writup is available on the [Altecoin Wiki](https://github.com/Altecoin-Project/Altecoin/wiki/ColdStaking), and an initial video showcase is available on [YouTube](https://www.youtube.com/watch?v=utxB5TzAeXc).
A brief guide to setup cold staking with GUI and RPC is available [here](https://github.com/random-zebra/Altecoin-Wiki/blob/master/User-Documentation/Cold-Staking-HowTo.md).

<sup>1</sup> Spending cold stakes from HW wallets currently available only for Ledger devices via [PET4L](https://github.com/Altecoin-Project/PET4L) tool.

Multi-Split Stake Splitting
-------------------

Stake splitting has received a makeover and now supports splitting to more than two (2) outputs. [PR #968](https://github.com/Altecoin-Project/Altecoin/pull/968) introduced the change, which is controlled by the wallet's `stakesplitthreshold` setting.

The default split threshold remains at 2000 ALTC, and can be adjusted in the GUI's Settings page, or via the RPC `setstakesplitthreshold` command.
...
### V1 zALTC Spending (Public Spends Version 4)

Since the discovery of a critical exploit within the libzerocoin library in early 2019, remaining legacy v1 zALTC have been un-spendable. We're happy to say that, once the new consensus rules are in effect, users will once again be able to spend their v1 zALTC with public spends version 4 ([PR #936](https://github.com/Altecoin-Project/Altecoin/pull/936)).

As with the previous version 3 public spends introduced in core wallet version 3.3.0 (enabling the spending of v2 zALTC), version 4 spends will also be public. A full technical writeup is available on the [Altecoin Wiki](https://github.com/Altecoin-Project/Altecoin/wiki/CoinRandomnessSchnorrSignature).
...
[#PR1022](https://github.com/Altecoin-Project/Altecoin/pull/1022) defines Version 7 blocks, which remove the (now-unused) accumulator checkpoint from the block header. This results in an overall data reduction of ~256 bits from each block as well as the in-memory indexes.

### New Network Message Signatures

Layer 2 network messages (MN, Budget, Spork, etc) are now signed based on the hash of their **binary** content instead of their **string** representation ([#PR1024](https://github.com/Altecoin-Project/Altecoin/pull/1024)).

### New SPORKS

Two new SPORKS are introduced, `SPORK_17` ([#PR975](https://github.com/Altecoin-Project/Altecoin/pull/975)) and `SPORK_18` ([#PR995](https://github.com/Altecoin-Project/Altecoin/pull/995)).<br>
...
- `delegated_balance` - ALTC balance held in P2CS contracts (delegated amount total).
- `cold_staking_balance` - ALTC balance held in cold staking addresses.
...
Returns a new Altecoin cold staking address for receiving delegated cold stakes.

Arguments:
1. "account"        (string, optional) The account name for the address to be linked to. if not provided, the default account "" is used. It can also be set to the empty string "" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.

Result:
"altecoinaddress"    (string) The new altecoin address
```
...
Arguments:
1. "stakingaddress"      (string, required) The altecoin staking address to delegate.
2. "amount"              (numeric, required) The amount in ALTC to delegate for staking. eg 100
3. "owneraddress"        (string, optional) The altecoin address corresponding to the key that will be able to spend the stake.
...
Result:
amount              (numeric) The total amount in ALTC received for this account in P2CS contracts.
```
...
[
   ...
   "address": "xxx",  (string) Altecoin address string
   }
  ...
]
```
...
For our linux users, in addition to the [Ubuntu PPA](https://launchpad.net/~altecoin) repository, we are now offering a [Snap package](https://snapcraft.io/altecoin-core) as quick way to install and update a Altecoin wallet.
...
- #936 `12a6b704b6` [zALTC] PublicCoinSpend v4 - Coin Randomness Schnorr Signature (random-zebra)
...
- #1129 `a87bfc32a0` [Consensus] Define TestNet changeover block for Altecoin v4.0 RC (random-zebra)
- #1134 `a4ded20de4` [Trivial] Remove a duplicate variable definition (warrows)
- #1191 `9a054eeba6` [Consensus] Define MainNet changeover block for Altecoin v4.0 (random-zebra)
...
- #1125 `c6e238ca4d` [GUI] Inform if open altecoin.conf and/or backups folder fail. (furszy)
...
- #1179 `7a33fe10d2` [Qt] Fix for dead link to wrong Altecoin website (NoobieDev12)
