Altecoin Core version *3.1.1* is now available from:  <https://github.com/altecoin-project/altecoin/releases>
...
Altecoin Core v3.1.1 is a non-mandatory update to address bugs and introduce minor enhancements that do not require a network change.
...
### Fix spending for v1 zALTC created before block 1050020

The transition to v2 zALTC and reset of the accumulators caused blocks 1050000 - 1050010 to be accumulated twice. This was causing a number v1 zALTC to not create valid witnesses, and thus were not spendable. This problem is fixed by double accumulating blocks 1050000-1050010 when creating the witness. Any user that had issues spending zALTC v1 will now be able to convert that into ALTC and then zALTC v2 (if desired).

### Adjustment to staking properties to reduce orphaned blocks

zALTC stake set to update more frequently and lowering the stake hashdrift to 30 seconds to reduce the number of orphans being experienced by Altecoin stakers.
...
### Core Features
 ...
 - #609 `6b73598b9` [MoveOnly] Remove zALTC code from main.cpp (presstab)
...
### GUI
 ...
 - #600 `217433561` [GUI] Only enable/disable PrivacyDialog zALTC elements if needed. (presstab)
...
### Wallet
 ...
 - #603 `779d8d597` Fix spending for v1 zALTC created before block 1050020. (presstab)
