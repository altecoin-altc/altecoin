    ...
    model(0)
{
    ...
    // Title
    ui->labelTitle->setText(tr("Select zTZT Denominations to Spend"));
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend zTZT.";
            else if (!mint.isSeedCorrect)
                strReason = "The zTZT seed used to mint this zTZT is not the same as currently hold in the wallet";
            else
