...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered tanzanite address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 TZT.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal tanzanite:URI (tanzanite:XyZ...?message=example)
            {
...
static void NotifyzTZTReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzTZTReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzTZTReset.connect(boost::bind(NotifyzTZTReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzTZTReset.disconnect(boost::bind(NotifyzTZTReset, this));
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose) {
    ...
    if (address.IsStakingAddress())
        return error("Invalid Tanzanite address, cold staking address");
    CKeyID keyID;
...
bool WalletModel::getKeyId(const CBitcoinAddress& address, CKeyID& keyID) {
    if (!address.IsValid())
        return error("Invalid Tanzanite address");

    if (!address.GetKeyID(keyID))
        return error("Unable to get KeyID from Tanzanite address");
