...
#include "qt/tanzanite/forms/ui_send.h"
#include "qt/tanzanite/addnewcontactdialog.h"
#include "qt/tanzanite/qtutils.h"
#include "qt/tanzanite/sendchangeaddressdialog.h"
#include "qt/tanzanite/optionbutton.h"
#include "qt/tanzanite/sendconfirmdialog.h"
...
SendWidget::SendWidget(TanzaniteGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (TZT)"));
    ...
    ui->labelAmountSend->setText("0.00 TZT");
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_TZT : AskPassphraseDialog::Context::Send_zTZT, true)){
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zTZT is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zTZT selector if applicable
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zTZT transaction sent!"));
        ZPivControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZTZT_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zTZT require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zTZT transaction
        ...
        emit message("zTZT transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isTZT){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zTZT in your balance to select."));
        }
    }
}
...
void SendWidget::onTZTSelected(bool _isTZT){
    isTZT = _isTZT;
    setCssProperty(coinIcon, _isTZT ? "coin-icon-piv" : "coin-icon-piv");
    refreshView();
