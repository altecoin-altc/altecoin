...
#include "qt/tanzanite/forms/ui_requestdialog.h"
#include <QListView>
    ...
    ui(new Ui::RequestDialog)
{
    ...
    ui->labelMessage->setText(tr("Instead of sharing only a Tanzanite address, you can create a payment request, bundling up more information."));
...
void RequestDialog::onNextClicked(){
    if(walletModel) {
        ...
        if (this->isPaymentRequest) {
            ...
            title = "Request for " + BitcoinUnits::format(displayUnit, value, false, BitcoinUnits::separatorAlways) + " TZT";
        } else {
