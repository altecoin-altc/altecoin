...
// Copyright (c) 2022 The Altecoin Developer
...
TopBar::TopBar(ALTECOINGUI* _mainWindow, QWidget* parent) : PWidget(_mainWindow, parent),
                                                        ui(new Ui::TopBar)
{
    ui->setupUi(this);
    ...
    setCssProperty({ui->labelAmountTopAltc}, "amount-small-topbar");
    setCssProperty({ui->labelAmountAltc}, "amount-topbar");
    setCssProperty({ui->labelPendingAltc, ui->labelImmatureAltc, ui->labelAvailableAltc,
                       ui->labelLockedAltc, ui->labelMasternodeCount, ui->labelCollateralAltc,
    ...
    ui->pushButtonConf->setButtonText("altecoin.conf");
...
void TopBar::refreshStatus()
{
    ...
    // Collateral
    ui->labelCollateralAltc->setText(GUIUtil::formatBalance(CMasternode::GetMasternodeNodeCollateral(chainActive.Tip()->nHeight), nDisplayUnit));
}
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // ALTC
    // Top
    ui->labelAmountTopAltc->setText(GUIUtil::formatBalance(nAvailableBalance, nDisplayUnit));
    // Expanded
    ui->labelAmountAltc->setText(GUIUtil::formatBalance(newBalance.balance + newBalance.immature_balance, nDisplayUnit));
    ui->labelAvailableAltc->setText(GUIUtil::formatBalance(nAvailableBalance, nDisplayUnit));
    ui->labelPendingAltc->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance, nDisplayUnit));
    ui->labelImmatureAltc->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
    ui->labelLockedAltc->setText(GUIUtil::formatBalance(nLockedBalance, nDisplayUnit));
