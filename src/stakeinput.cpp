...
// The zTZT block index is the first appearance of the accumulator checksum that was used in the spend
...
bool CZPivStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zTZT staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
{
    //The unique identifier for a zTZT is a hash of the serial
...
{
    //Create an output returning the zTZT that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZTZTOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zTZT output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zTZT", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZTZTOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zTZT output", __func__);
...
{
    CzTZTTracker* zpivTracker = pwallet->zpivTracker.get();
    CMintMeta meta;
...
{
    //The unique identifier for a TZT stake is the outpoint
