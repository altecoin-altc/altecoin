...
void AddTimeData(const CNetAddr& ip, int64_t nOffsetSample, int nOffsetLimit)
{
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1) {
        ...
        if (abs64(nMedian) < nOffsetLimit) {
            ...
            std::string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Tanzanite Core will not work properly.");
