...
CzTZTWallet::CzTZTWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZTZTSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZTZTSeed_deprecated()) {
                    LogPrintf("%s: Updated zTZT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzTZTWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZTZTCount(nCountLastUsed);
    else if (!walletdb.ReadZTZTCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzTZTWallet::Lock()
{
    seedMaster = 0;
}

void CzTZTWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzTZTWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZTZT(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzTZTWallet::LoadMintPoolFromDB()
{
...
void CzTZTWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzTZTWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzTZTWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzTZTWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZTZT(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzTZTWallet::SeedToZTZT(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzTZTWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzTZTWallet::GenerateDeterministicZTZT(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzTZTWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzTZTWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzTZTWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
