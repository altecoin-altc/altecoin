...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zpivTracker = std::unique_ptr<CzTZTTracker>(new CzTZTTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZTZT_TRX_FUNDS_PROBLEMS, strprintf("Failed Invalid Amount Check: %s", receipt.GetStatusMessage()));
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZTZT_TRX_FUNDS_PROBLEMS, strprintf("Failed Invalid Amount Check: %s", receipt.GetStatusMessage()));
...
BOOST_AUTO_TEST_CASE(zerocoin_public_spend_test)
{
    ...
    // -----------------------------------
    int spendVersion = 3;
    BOOST_CHECK_MESSAGE(ZTZTModule::createInput(in1, mint_v2, tx1.GetHash(), spendVersion),
    ...
    BOOST_CHECK_MESSAGE(ZTZTModule::validateInput(in1, out_v2, tx1, publicSpend1),
    ...
    BOOST_CHECK_MESSAGE(!ZTZTModule::validateInput(in1, out_v2, tx1, publicSpend1b), "Different denomination for mint v2 and spendVersion 3");

    // check spendVersion = 4 for v2 coins
    // -----------------------------------
    spendVersion = 4;
    BOOST_CHECK_MESSAGE(ZTZTModule::createInput(in2, mint_v2, tx2.GetHash(), spendVersion),
    ...
    BOOST_CHECK_MESSAGE(ZTZTModule::validateInput(in2, out_v2, tx2, publicSpend2),
            "Failed to validate zc input for mint v2 and spendVersion 4");

    // Verify that it fails with a different denomination
    in2.nSequence = 500;
    PublicCoinSpend publicSpend2b(ZCParams_v2);
    BOOST_CHECK_MESSAGE(!ZTZTModule::validateInput(in2, out_v2, tx2, publicSpend2b), "Different denomination for mint v2 and spendVersion 4");

    // check spendVersion = 4 for v1 coins
    // -----------------------------------
    BOOST_CHECK_MESSAGE(ZTZTModule::createInput(in3, mint_v1, tx3.GetHash(), spendVersion),
    ...
    BOOST_CHECK_MESSAGE(ZTZTModule::validateInput(in3, out_v1, tx3, publicSpend3),
            "Failed to validate zc input for mint v1 and spendVersion 4");

    // Verify that it fails with a different denomination
    in3.nSequence = 500;
    PublicCoinSpend publicSpend3b(ZCParams_v1);
    BOOST_CHECK_MESSAGE(!ZTZTModule::validateInput(in3, out_v1, tx3, publicSpend3b), "Different denomination for mint v1 and spendVersion 4");
