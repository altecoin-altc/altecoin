...
TMPDIR_PREFIX = "tanzanite_func_test_"


class TanzaniteTestFramework():
    """Base class for a tanzanite test script.

    Individual tanzanite test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave tanzaniteds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop tanzaniteds after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__))+"/../../../src"),
                          help="Source directory containing tanzanited/tanzanite-cli (default: %default)")
        ...
        parser.add_option("--usecli", dest="usecli", default=False, action="store_true",
                          help="use tanzanite-cli instead of RPC for all commands")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: tanzaniteds were not stopped and may still be running")
    ...
    def stop_node(self, i):
        """Stop a tanzanited test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple tanzanited test nodes"""
        for node in self.nodes:
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None, *args, **kwargs):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'tanzanited exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "tanzanited should have exited with an error"
                else:
                    assert_msg = "tanzanited should have exited with expected error " + expected_msg
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as tanzanited's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self, toPosPhase=False):
        ...
        def copy_cachedir(origin, destination, num_nodes=MAX_NODES):
            for i in range(num_nodes):
                ...
                initialize_datadir(destination, i)  # Overwrite port/rpcport in tanzanite.conf
        ...
        def clone_cache_from_node_1(cachedir, from_num=4):
            ...
            for i in range(from_num, MAX_NODES):
                ...
                initialize_datadir(cachedir, i)  # Overwrite port/rpcport in tanzanite.conf
        ...
        def start_nodes_from_dir(ddir, num_nodes=MAX_NODES):
            ...
            for i in range(num_nodes):
                ...
                args = [os.getenv("BITCOIND", "tanzanited"), "-spendzeroconfchange=1", "-server", "-keypool=1",
        ...
        def generate_pow_cache():
            ...
            # Create cache directories, run tanzaniteds:
        ...
        if not toPosPhase and not is_powcache_valid:
            ...
            # - Nodes 2 and 3 mint one zerocoin for each denom (tot 6666 TZT) on block 301/302
            ...
            for peer in range(4):
                for j in range(20):
                    ...
                    if nBlocks == 301 or nBlocks == 302:
                        # mints 7 zerocoins, one for each denom (tot 6666 TZT), fee = 0.01 * 8
                        # consumes 27 utxos (tot 6750 TZT), change = 6750 - 6666 - fee
    ...
    ### Tanzanite Specific TestFramework ###
    ###################################
    ...
    def test_PoS_chain_balances(self):
        ...
        # - Nodes 2 and 3 have 6666 TZT worth of zerocoins
    ...
    def stake_block(self, node_id,
        ...
        for tx in vtx:
            if not fDoubleSpend:
                # assume txes don't double spend zTZT inputs when fDoubleSpend is false. It needs to
...
class ComparisonTestFramework(TanzaniteTestFramework):
    """Test framework for doing p2p comparison testing

    Sets up some tanzanited binaries:
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("BITCOIND", "tanzanited"),
                          help="tanzanited binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("BITCOIND", "tanzanited"),
                          help="tanzanited binary to use for reference nodes (if any)")
